...
type Person = { id: string; name: string; profileImage: string };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setPeople([]); // 더미 데이터 없음 → 비어있는 상태로 표시
      }
    })();
  }, []);
    ...
    <ul className="space-y-4">
      {people.map((p) => (
        <li key={p.id} className="flex items-center gap-3">
          <Image src={p.profileImage} alt={p.name} width={48} height={48} className="rounded-full" />
